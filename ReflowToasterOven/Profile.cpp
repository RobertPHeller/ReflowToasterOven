...
void Profile::Load() {
    ...
    for (i = 0; i < sizeof(profile_t); i++) {
        uint8_t j = EEPROM.read(EEPROM_PROFILE_ADDR + i);
        ptr[i] = j;
        checksum ^= j; // track checksum to verify later
    }
    // verify data validity
    uint8_t k = EEPROM.read(EEPROM_PROFILE_ADDR + i);
    if (checksum != k || !Valid()) {
...
void Profile::Save() {
    ...
    for (i = 0; i < sizeof(profile_t); i++) {
        uint8_t j = ptr[i];
        EEPROM.write(EEPROM_PROFILE_ADDR + i, j);
        checksum ^= j; // track checksum to save
    }
    EEPROM.write(EEPROM_PROFILE_ADDR + i, checksum); // save the checksum for validation during load
}
